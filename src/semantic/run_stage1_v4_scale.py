...
def calculate_adaptive_scale(
    ...
    """
    Adaptive scale calculation based on mask area ratio.
    Formula: rho = (Area_mask / Area_total)^gamma, Scale = rho * eta
    """
    if not location_prompt:
        return 0.5 

    # 1. Extract Heatmap / Mask (Low threshold to capture general area)
    ...
    # 2. Calculate Area Ratio (r)
    mask_binary = (heatmap > 0.1).float()
    area_pixels = mask_binary.sum().item()
    total_pixels = H * W
    r = area_pixels / total_pixels
    
    if r < 1e-5:
        print(f"  [Adaptive] No active area found for '{location_prompt}', using default.")
        return 0.5

    # 3. Calculate Side Length Ratio (rho)
    rho = r ** gamma
    
    # 4. Calculate Suggested Scale
    suggested_scale = rho * eta
    
    # 5. Clip
    final_scale = max(min_scale, min(suggested_scale, max_scale))
    
    print(f"  [Adaptive] Area Ratio: {r:.4f} -> Rho: {rho:.4f} -> Calc Scale: {suggested_scale:.4f} -> Final: {final_scale:.4f}")
...
def stage1_composition(
    ...
    for gscale in global_scale_factors:
        ...
        for item in encoded_objects:
            ...
            # -------------------------------------------------------------
            #  MODIFIED: Adaptive Scale Logic
            # -------------------------------------------------------------
            if use_adaptive and location_prompt:
                print(f"Object: {conf['text']} | Calculating adaptive scale based on '{location_prompt}'...")
                ...
                # Generate candidates around the adaptive scale
                candidate_scales = [
                    adaptive_sc * 0.9,
                    adaptive_sc * 1.0,
                    adaptive_sc * 1.1
                ...
                # Original logic: Global scale * Base scale
...
if __name__ == "__main__":
    ...
    output_path = "assets/group1/stage1_result2/"
    ...
    objects_to_add = [
        ...
        },
        {
            ...
            # Enable Adaptive Scale for the second object
